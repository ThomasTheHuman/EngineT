...
void PrefabManager::createPlayer(Vector2D position)
{
	...
	player.addComponent<HealthComponent>(10, 10, true);
...
void PrefabManager::createEnemy(Vector2D position)
{
	...
	enemy.addComponent<HealthComponent>(4, 4, false);
...
void PrefabManager::createEnemy2(Vector2D position)
{
    ...
    enemy.addComponent<ForceComponent>(force);
    enemy.addComponent<HealthComponent>(10, 10, false);
    enemy.addComponent<AnimationComponent>();
    enemy.getComponent<AnimationComponent>().addAnimation(1, Vector2D(0, 0), 4); //idle
    enemy.getComponent<AnimationComponent>().addAnimation(2, Vector2D(0, 1), 8); //run
    enemy.getComponent<AnimationComponent>().addAnimation(3, Vector2D(5, 4), 1); //jump
...
void PrefabManager::createEyeball(Vector2D position)
{
    ...
    enemy.addComponent<ForceComponent>(force);
    enemy.addComponent<HealthComponent>(2, 2, false);
    enemy.addComponent<AnimationComponent>();
...
void PrefabManager::createMap(const Map& map, const char * spriteSheet, Vector2D offset)
{
	Vector2D position;
	for(int object : map.background)
	{
		if(object)
		{
			...
			background.addGroup(0);
			background.addComponent<TransformComponent>(*new Vector2D(position.x + offset.x, position.y + offset.y));
	...
	for (int object : map.foreground)
	{
		if (object)
		{
			...
			Vector2D srcPosition((object % 23) - 1, (object / 23));
			foreground.addComponent<TransformComponent>(*new Vector2D(position.x + offset.x, position.y + offset.y));
			...
			} else if (std::find(map.chain.begin(), map.chain.end(), object) != map.chain.end())
			{
				foreground.addGroup(2);
			} else {
			    foreground.addGroup(3);
			}
		} else {
		    if (!(bool)(rand()%500))
		    { //TODO
		        if(!(bool)(rand()%2)) {
		            if(!(bool)(rand()%2)) {
		                createEnemy(*new Vector2D(position.x + offset.x, position.y + offset.y));
		            } else {
		                createEnemy2(*new Vector2D(position.x + offset.x, position.y + offset.y));
		            }
		        } else {
		            createEyeball(*new Vector2D(position.x + offset.x, position.y + offset.y));
		        }
		    }
		}
		position.x += 16;
