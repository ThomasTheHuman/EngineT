...
void CollisionComponent::update()
{
	...
	if(collisionCount)
	{
		...
		if (collisions.x < collisions.y) //right-left
		{
			...
			if (entity->hasComponent<HealthComponent>() && entity->getComponent<TransformComponent>().velocity.x > 4) {
			    entity->getComponent<HealthComponent>().damage(entity->getComponent<TransformComponent>().velocity.x);
			}
		...
		} else if (collisions.x > collisions.y) //top-down
		{
			...
			if (entity->hasComponent<HealthComponent>() && entity->getComponent<TransformComponent>().velocity.y > 4) {
			    entity->getComponent<HealthComponent>().damage(entity->getComponent<TransformComponent>().velocity.y);
			}
